...
| FR-6 | Integrated Workflow & Orchestration (P1) | ✅ 2.7: stage machine now matches the PRD 7-stage lifecycle (**submission intake → eligibility check → NOTA drafting → internal review → board preparation → decision → communication dispatch** → closed) with per-stage entry/exit conditions + owner exposed at `/api/workflow/stages`; risk-tier human checkpoints (internal review / board prep / decision); authenticated sign-offs; legacy-stage migration; Postgres-persisted; 403/identity tested | Still pending: stage owners assignable, task notifications, parallel workstreams + merge gates, SLA bottleneck alerts, live status dashboard | ✅ 2.7; 4.7, 5.1 | **Sprint 4** (dashboard 5.1) |
