...
| 2.10 | **Document understanding pipeline**: Granite-Docling structure parsing (layout, tables, reading order, clause structure → structured markdown) + Tesseract OCR (`ind`+`eng`) for scanned pages; routing born-digital → Docling, scanned → Tesseract → Docling, low-confidence → multimodal role; feeds semantic chunking (2.2) | FR-1 ingestion, NFR data residency (fully local) | Scanned Bahasa Indonesia submission produces a structured, chunkable text layer; clause structure preserved end-to-end; pipeline runs with zero external calls |
