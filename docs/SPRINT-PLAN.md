...
| 2.9 | **Multi-model router (both tiers per role)** behind `LLMGateway` per [config/models.yaml](../config/models.yaml): online API — extraction/routing `claude-haiku-4-5`, **drafting `claude-sonnet-4-6`** (→ `claude-opus-4-8` for board-bound, risk-tier gated), reasoning/consistency `claude-opus-4-8`, multimodal `claude-opus-4-8` vision; self-host — Qwen3-235B-A22B/32B, DeepSeek-R1-Distill-Qwen-32B, Mistral Small 3.x, Gemma 3 27B (→ Gemma 4); vLLM OpenAI-compatible serving adapter; **prompt caching on stable system/context prefixes**; policies RTE-001…005 (outage fallback, classification-forced tier, per-call audit, eval-gated activation, edge profile pins self-host). Cost model: [COST-MODEL.md](COST-MODEL.md) | FR-5, §11.2, R1/R2 | Router selects by role + tier policy; cheapest-tier-that-clears-the-bar per role; cache hit-rate >0 on repeated prefixes; any model entering a role passes grounding gate + 2.4 regression suite; every call audited |
| 2.10 | **Document understanding pipeline** — **Docling primary, Tesseract fallback OCR, multimodal escalation**: Granite-Docling does structure (layout, tables, reading order, clauses) **and** OCR → structured markdown; Tesseract (`ind`+`eng`, printed **and** handwriting/ICR per addendum A2) is the fallback OCR engine when Docling confidence is low; still-low-confidence/image-heavy/handwritten pages escalate to the multimodal role; feeds semantic chunking (2.2). Order: born-digital → Docling only; scanned → Docling OCR → Tesseract → multimodal | FR-1 ingestion, NFR data residency (fully local), addendum A2 | Scanned Bahasa Indonesia submission (incl. handwriting) produces a structured, chunkable text layer; clause structure preserved; pipeline runs with zero external calls |
