...
class PostgresCaseRepository(CaseRepository):
    ...
    async def save(self, case: Case) -> None:
        async with self._engine.begin() as conn:
            await conn.execute(
                text(
                    "INSERT INTO cases (case_id, title, stage, risk_tier, history, created_at) "
                    "VALUES (:id, :title, :stage, :tier, :history, :created) "
        ...
        async with self._engine.begin() as conn:
            await conn.execute(
                text(
                    ...
                    "(id, case_id, doc_type, title, version, is_master, parent_doc_id, "
                    " storage_key, created_at) "
                    "VALUES (:id, :case_id, :doc_type, :title, :version, :is_master, "
                    "        :parent, :key, :created) "
                    "ON CONFLICT (id) DO UPDATE SET "
                    "version = :version, storage_key = :key"
                ),
                {
                    "id": document.id,
